...
public class MailUtil {
	 ...
	 * @param title     标题
	 * @param content   内容
	 * @param reAccount 收件人邮箱
	 */
	public MailUtil(String title, String content, String reAccount) {
		receiveMailAccount = reAccount;
		this.title = title;
	...
	private final String myEmailAccount = ConfigMapUtil.getValueByKey("mail.account");
	private final String myEmailPassword = ConfigMapUtil.getValueByKey("mail.password");

	// 发件人邮箱的 SMTP 服务器地址, 必须准确, 不同邮件服务器地址不同, 一般(只是一般, 绝非绝对)格式为: smtp.xxx.com
	// 网易126邮箱的 SMTP 服务器地址为: smtp.126.com
	private final String myEmailSMTPHost = ConfigMapUtil.getValueByKey("mail.smtp");
	...
	private final String mailname = ConfigMapUtil.getValueByKey("mail.mailname");
	...
	public MimeMessage createMimeMessage(Session session) throws Exception {
		...
		// 2. From: 发件人
		message.setFrom(new InternetAddress(myEmailAccount, mailname, "UTF-8"));

		// 3. To: 收件人（可以增加多个收件人、抄送、密送）
		message.setRecipient(MimeMessage.RecipientType.TO,
				new InternetAddress(receiveMailAccount, receivename, "UTF-8"));
