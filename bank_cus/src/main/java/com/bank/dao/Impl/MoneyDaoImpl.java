...
public class MoneyDaoImpl extends AbstractDAO implements MoneyDao{
	...
	//余额表
	public static String tablename = "money";

	public MoneyDaoImpl() {
	};

	public MoneyDaoImpl(String cid) {
		try {
			super.setEnv(AppConfig.getApp(), cid, tablename, new Money());
	...
	@Override
	public List<Money> getList(Money pojo) {
		String where = "";
		if (pojo != null ) {
	...
	@Override
	public Boolean insertOne(Money pojo) {
	...
	@Override
	public Integer updateOne(Money pojo) {
	...
	@Override
	public Integer deleteOne(Money pojo) {
		try {
	...
	@Override
	public Pagehelper<Money> getPagehelper(Money pojo, Integer page, Integer size) {
		Pagehelper<Money> pagehelper = new Pagehelper<Money>();
		String where = "";
		if(pojo != null) {
			if(pojo.id != null) {
				where = vig.core.util.SQLUtil.andCondition(where, vig.core.util.SQLUtil.composeSQL("id", pojo.id));
		...
		try {
			...
			return (Pagehelper<Money>) PageReturn.pageReturn(pagehelper);
		} catch (VIGException e) {
