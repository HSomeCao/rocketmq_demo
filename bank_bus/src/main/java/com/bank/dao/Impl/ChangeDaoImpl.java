 ...
 * 2019年4月26日
 */
public class ChangeDaoImpl extends AbstractDAO implements ChangeDao{

	//换卡表
	public static String tablename = "change";

	public ChangeDaoImpl() {
	};

	public ChangeDaoImpl(String cid) {
		try {
			super.setEnv(AppConfig.getApp(), cid, tablename, new Change());
	...
	@Override
	public List<Change> getList(Change pojo) {
		String where = "";
		if (pojo != null ) {
	...
	@Override
	public Boolean insertOne(Change pojo) {
	...
	@Override
	public Integer updateOne(Change pojo) {
	...
	@Override
	public Integer deleteOne(Change pojo) {
		try {
	...
	@Override
	public Pagehelper<Change> getPagehelper(Change pojo, Integer page, Integer size) {
		Pagehelper<Change> pagehelper = new Pagehelper<Change>();
		String where = "";
		if(pojo != null) {
			if(pojo.id != null) {
				where = vig.core.util.SQLUtil.andCondition(where, vig.core.util.SQLUtil.composeSQL("id", pojo.id));
		...
		try {
			...
			return (Pagehelper<Change>) PageReturn.pageReturn(pagehelper);
		} catch (VIGException e) {
