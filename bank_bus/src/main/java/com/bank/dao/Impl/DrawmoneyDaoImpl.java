...
import com.bank.dao.DrawmoneyDao;
 ...
 * 2019年4月26日
 */
public class DrawmoneyDaoImpl extends AbstractDAO implements DrawmoneyDao{

	//取钱表
	public static String tablename = "drawmoney";

	public DrawmoneyDaoImpl() {
	};

	public DrawmoneyDaoImpl(String cid) {
		try {
			super.setEnv(AppConfig.getApp(), cid, tablename, new Drawmoney());
	...
	@Override
	public List<Drawmoney> getList(Drawmoney pojo) {
		String where = "";
		if (pojo != null ) {
	...
	@Override
	public Boolean insertOne(Drawmoney pojo) {
	...
	@Override
	public Integer updateOne(Drawmoney pojo) {
	...
	@Override
	public Integer deleteOne(Drawmoney pojo) {
		try {
	...
	@Override
	public Pagehelper<Drawmoney> getPagehelper(Drawmoney pojo, Integer page, Integer size) {
		Pagehelper<Drawmoney> pagehelper = new Pagehelper<Drawmoney>();
		String where = "";
		if(pojo != null) {
			if(pojo.id != null) {
				where = vig.core.util.SQLUtil.andCondition(where, vig.core.util.SQLUtil.composeSQL("id", pojo.id));
		...
		try {
			...
			return (Pagehelper<Drawmoney>) PageReturn.pageReturn(pagehelper);
		} catch (VIGException e) {
